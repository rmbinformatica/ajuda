...
#### Configurar o Git para assinar commits automaticamente
...
Defina o caminho do gpg:

No windows, você pode confirmar o caminho executando o comando `where.exe gpg`.

```bash
git config --global gpg.program "C:/Program Files/GnuPG/bin/gpg.exe"
```

No linux, você pode confirmar o caminho executando o comando `which gpg`.
```bash
